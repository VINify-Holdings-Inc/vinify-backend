  ...
  try {
      ...
      const queryBuilder = VehicleData.createQueryBuilder("vd")
          .select([
              ...
              "masterbrand.name AS brand"  // Get the brand name from the masterbrand table
          ])
          .leftJoin(MasterState, "masterstate", "vd.state = masterstate.code") // Join masterstate for state names
          .leftJoin(MasterBrand, "masterbrand", "vd.brand = masterbrand.code") // Join masterbrand for brand names
  ...
  try {
      ...
      const queryBuilder = VehicleData.createQueryBuilder("vehicle")
          .select([
              ...
              "masterbrand.name AS brand" // Select the brand name from masterbrand table
          ])
