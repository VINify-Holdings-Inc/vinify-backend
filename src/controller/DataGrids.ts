...
export const DashboardSummaryVIN = async (req: any, res: any) => {
  try {
    ...
    const queryBuilder = VehicleData.createQueryBuilder("vehicle")
      ...
      .distinctOn(["vehicle.vin"]) // Ensure VIN is used for distinct selection
      .orderBy("vehicle.titleBrandDate", "DESC") // First order by titleBrandDate to get the latest entry
      .addOrderBy("vehicle.vin", "ASC") // Then order by VIN to maintain distinct order
      .limit(limit)
      .offset(offset);
