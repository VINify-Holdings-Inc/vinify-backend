...
Customer-managed-key duties are separated through IAM Identity Center. Betty's standing `Engineering-Daily` role cannot administer KMS keys; a live `kms:PutKeyPolicy` negative test returned `AccessDenied` on 2026-08-15. Bethanie holds the scoped, one-hour `KMS-Key-Administrator` permission set for only `alias/vin` and `alias/WAF`, with cryptographic use explicitly denied. Neither person has standing `SystemAdministrator`; that permission set is attached only to the empty-by-default, automatically revoked `AWS-Elevated-Admin-JIT` group. See [Key-Management-Policy.md](Key-Management-Policy.md) Section 5.
