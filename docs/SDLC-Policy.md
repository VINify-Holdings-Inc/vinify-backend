...
- Passwords are hashed with bcrypt at a work factor of 12, with a per-password salt generated by bcrypt, never stored in plaintext. The work factor is defined once as `BCRYPT_COST` in `src/controller/LoginController.ts` and used by every write path. The earlier plaintext scheme was migrated away from (git history, `security/bcrypt-password-hashing`); its compatibility branch was removed on 2026-09-10 once the last legacy account had been upgraded, so no plaintext comparison path remains in the codebase.
...
**Controller-level coverage (added 2026-08-08; corrected 2026-09-10):** `DataRetentionCronJob` is covered (deletion order — `Login` before `User` — and the exact 90-day/2-year cutoff windows), as is `auditLog` record structure, using `jest.mock` on the TypeORM entities rather than a live test database. **23 tests across three suites, all passing** — `utils.test.ts` (15), `DataRetentionCronJob.test.ts` (5), `auditLog.test.ts` (3).

An earlier version of this section claimed `LoginController` was covered (bcrypt login, the legacy-plaintext upgrade path, the closed-account block, `CloseAccount`) and gave a total of 31 tests. That was inaccurate: no `LoginController` test file exists and the total is 23. Corrected here rather than left standing. `LoginController` remains uncovered, along with `ForgetPassword`/`ResetPassword`/`ProfileUpdate`/`userProfileUpdate` and any route requiring a real file upload or email send — real, ongoing scope, not claimed as done.
