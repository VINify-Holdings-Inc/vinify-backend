...
| 1.5 | 2026-09-06 | Betty Waiyego (Engineering Lead) | Records a production backup-plan configuration correction: removed a redundant AWS Backup `CopyAction` that copied each daily `mvmprod` recovery point back into the same locked `Default` vault in `us-east-1`. The primary daily backup schedule, 35-day immutable retention, 14-day RDS point-in-time recovery, and cross-region automated-backup replication remain enabled. Existing locked recovery points were unchanged and continue to expire under their enforced retention periods. |
| 1.6 | 2026-09-07 | Betty Waiyego (Engineering Lead) | Records the S3 cross-region DR footprint as it actually stands. Replication to `eu-west-2` (London) was configured earlier and then withdrawn on data-residency grounds; production replication is `us-west-1` only, for both `sensitivedatavin` and `vinify-frontend-deploy-artifacts`. The eu-west-2 grants have now been removed from both replication IAM policies, so the decision is enforced technically rather than by intent. The two London buckets cannot be deleted yet — Object Lock in COMPLIANCE mode holds their contents until August/September 2027 — but they hold only a 339-byte compliance test file and public frontend assets (fonts, icons, images), no client or personal data, and their existing lifecycle rules will clear them once retention lapses. |
